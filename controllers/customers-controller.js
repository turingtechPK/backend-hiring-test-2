...
const CUSTOMERS = [
  new Customer(1, "Arisha Barron", 500),
  new Customer(2, "Branden Gibson", 1500),
  new Customer(3, "Rhonda Church", 2500),
  new Customer(4, "Georgina Hazel", 4500),
];
...
const balanceInquiry = (req, res) => {
  ...
  } else res.status(404).json({ Error: "No account found with this id!" });
};
...
const transferHistory = (req, res) => {
  ...
  if (customer) {
    if (customer.transferHistory.length > 0) {
      ...
      res
        .status(401)
        .json({ Error: "No transfer history found for this account!" });
    }
  } else res.status(404).json({ Error: "No account found with this id!" });
};
